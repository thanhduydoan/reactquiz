...
function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/questions').then(res => res.json())
  ...
  return (
    <div className='app'>
      ...
      <Main>
        ...
        {status === "active" && <>
          <Progress index={index} numQuestions={numQuestions} points={points} maxPossiablepoints={maxPossiablepoints} answer={answer} />
        ...
        {status === 'finished' && <FinishScreen points={points} maxPossiblePoints={maxPossiablepoints} highscore={highscore} dispatch={dispatch} />}
      </Main>
    </div>
